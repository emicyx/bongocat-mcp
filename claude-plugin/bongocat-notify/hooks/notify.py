"""bongocat-notify hook：Claude Code 关键事件 → 猫咪气泡播报 + 表情切换。

与 ZCode 版（zcode-plugin/bongocat-notify/hooks/notify.py）同源，差异仅在
Claude Code 的事件模型（改代码前请先读完）：

- Claude Code 没有 PermissionRequest 事件：等待审批由 Notification 事件表达，
  message 形如 "Claude needs your permission to use Bash"；空闲 60 秒的
  "waiting for your input" 提示也走同一事件，须按关键词过滤，只播审批。
- Claude Code 没有 PostToolUseFailure 事件：工具出错要靠 PostToolUse 的
...
其余设计约束与 ZCode 版一致：
- 只用 Python 标准库。hook 由 Claude Code 以独立进程拉起，通过 bongocat-mcp
...
- hook 绝不能干扰会话：任何失败（仪表盘没开 / 猫下线 / 配置坏了）都
  记日志后静默 exit 0，不向 stdout 输出任何东西（UserPromptSubmit /
  SessionStart 的 stdout 会被注入上下文，空输出才安全）。
...
事件 JSON 由 Claude Code 从 stdin 喂入（字段不保证存在，一律防御性读取）。
配置覆盖：插件目录或 ~/.claude/ 下的 bongocat-notify.json（见 DEFAULTS）。
"""
...
DEFAULTS = {
    ...
    "events": {
        ...
        "stop": {
            "enabled": True,
            "text": "✅ 任务完成啦！Claude Code 已交出结果，快来看看喵～",
            "expressions": ["星星眼", "点赞", "开心", "高兴", "笑", "star", "happy"],
        },
        "approval": {
            "enabled": True,
            "text": "❓ Claude Code 在等你的审批：{tool}需要确认，喵？",
            "expressions": ["疑问", "疑惑", "问号", "?"],
        },
        "failure": {
            "enabled": True,
            "text": "💦 有工具出错了（{tool}），Claude Code 正在想办法喵…",
...
def response_is_error(resp) -> bool:
    ...
    Claude Code 各工具的错误形态不统一：MCP 工具是 dict 带 is_error/isError，
    Bash 失败是 "Exit code N" 开头的文本，其余多为 "Error..." 开头。
    """
    if resp is None:
        return False
...
def main() -> int:
    ...
    if event == "approval":
        # Notification 事件既报「等审批」也报「空闲等输入」，只播审批
        message = str(stdin.get("message") or "")
        if "permission" not in message.lower():
            return 0
        m = re.search(r"permission to use (\S+)", message)
